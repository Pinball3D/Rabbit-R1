...
public final /* data */ class TransferParameters {
    ...
    public TransferParameters(double d, double d2, double d3, double d4, double d5, double d6, double d7) {
        ...
        if (d5 < BuildConfig.SENTRY_SAMPLE_RATE || d5 > 1.0d) {
            throw new IllegalArgumentException("Parameter d must be in the range [0..1], was " + d5);
        }
        if (d5 == BuildConfig.SENTRY_SAMPLE_RATE && (d2 == BuildConfig.SENTRY_SAMPLE_RATE || d == BuildConfig.SENTRY_SAMPLE_RATE)) {
            throw new IllegalArgumentException("Parameter a or g is zero, the transfer function is constant");
        }
        if (d5 >= 1.0d && d4 == BuildConfig.SENTRY_SAMPLE_RATE) {
            throw new IllegalArgumentException("Parameter c is zero, the transfer function is constant");
        }
        if ((d2 == BuildConfig.SENTRY_SAMPLE_RATE || d == BuildConfig.SENTRY_SAMPLE_RATE) && d4 == BuildConfig.SENTRY_SAMPLE_RATE) {
            throw new IllegalArgumentException("Parameter a or g is zero, and c is zero, the transfer function is constant");
        }
        if (d4 < BuildConfig.SENTRY_SAMPLE_RATE) {
            throw new IllegalArgumentException("The transfer function must be increasing");
        }
        if (d2 < BuildConfig.SENTRY_SAMPLE_RATE || d < BuildConfig.SENTRY_SAMPLE_RATE) {
