...
final /* data */ class RichTooltipPositionProvider implements PopupPositionProvider {
    ...
    public long mo884calculatePositionllwVHH4(IntRect anchorBounds, long windowSize, LayoutDirection layoutDirection, long popupContentSize) {
        ...
        if (IntSize.m5043getWidthimpl(popupContentSize) + right > IntSize.m5043getWidthimpl(windowSize) && (right = anchorBounds.getLeft() - IntSize.m5043getWidthimpl(popupContentSize)) < 0) {
            right = anchorBounds.getLeft() + ((anchorBounds.getWidth() - IntSize.m5043getWidthimpl(popupContentSize)) / 2);
        }
